...
namespace Tabs { namespace Internal {

template <typename _Tp>
  constexpr bool is_signed_v = std::is_signed<_Tp>::value;

template <typename T,
          std::enable_if_t<is_signed_v<T>, std::nullptr_t> * = nullptr>
...
TabBar::TabBar(QWidget *parent) noexcept : QTabBar(parent) {
    ...
    auto *sessionManager = Core::SessionManager::instance();
...
void TabBar::onEditorOpened(Core::IEditor *editor) noexcept {
    ...
    this->setTabIcon(
        index,
        Utils::FileIconProvider::icon(document->filePath()));
